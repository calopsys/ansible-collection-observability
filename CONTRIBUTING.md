...
```bash
git clone git@github.com:calopsys/ansible-collection-observability.git ~/git/ansible_collections/calopsys/observability
```
...
```bash
make molecule.create role=alloy      # Create test instance
make molecule.converge role=alloy    # Apply the role
make molecule.verify role=alloy      # Run verification tests
make molecule.idempotence role=alloy # Verify idempotency
make molecule.destroy role=alloy     # Destroy test instance
```
...
```bash
cd ~/bla
ansible-galaxy collection install ~/git/ansible_collections/calopsys/observability/build/calopsys-observability-1.0.0.tar.gz -p ./collections
```
