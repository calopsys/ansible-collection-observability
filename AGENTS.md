...
This is an Ansible collection for the `calopsys` namespace that installs and manages observability stack components (Grafana LGTM: Loki, Grafana, Mimir + Alloy). Collections are installed at:
```
...
| Role | Description |
|------|-------------|
| `alloy` | Grafana Alloy - unified telemetry collector (metrics, logs, traces) |
| `grafana` | Grafana - visualization and analytics platform |
| `loki` | Grafana Loki - log aggregation system |
| `mimir` | Grafana Mimir - metrics backend (Prometheus-compatible) |
...
Use consistent tag naming: `calopsys_<role>_<action>`

Example:
```yaml
tags: ["calopsys_alloy_install"]
tags: ["calopsys_alloy_configure"]
tags: ["calopsys_mimir_services"]
```

### Variable Naming

- Use `snake_case` for variable names
- Prefix variables with `calopsys_<role>_` (e.g., `calopsys_alloy_user`)
- Example: `calopsys_mimir_config_dir`, `calopsys_loki_env_dir`
...
- name: "Task with condition."
  ...
  when: "calopsys_alloy_enable_feature | bool"
```
